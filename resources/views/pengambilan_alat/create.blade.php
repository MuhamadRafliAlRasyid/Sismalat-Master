        ...
        <div
            ...
            <form method="POST" action="{{ route('pengambilan_alat.store') }}" enctype="multipart/form-data"
                ...
                <div class="space-y-6">
                    ...
                    <div class="form-group" x-data="alatSelector(
                        {{ \Illuminate\Support\Js::from($alatsGrouped) }},
                        @js($selectedHashid ?? '')
                    )" x-init="console.log('Alat Selector initialized:', { folders: folders.length, preselected: selectedHashid })">
                        ...
                        <input type="hidden" name="alat_id" :value="selectedHashid" required>
                        ...
                        <div
                            ...
                            <div class="max-h-80 overflow-y-auto">
                                                    ...
                                                    <div class="flex-1 min-w-0">
                                                        <div class="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span class="text-xs text-gray-400"
                                                                x-show="item.tipe && item.tipe !== '-' && item.tipe !== ''"
                                ...
                                <div x-show="filteredFolders.length === 0" class="p-8 text-center text-gray-400">
                                    ...
                                    <button type="button" x-show="search" @click="search = ''"
                                        class="mt-2 text-amber-600 hover:text-amber-700 text-sm font-medium">
                                        Clear search
                                    </button>
                                </div>
                            </div>
        ...
        <script>
            function alatSelector(folders, preselectedHashid) {
                return {
                    ...
                    init() {
                        console.log('Alat Selector Init:', {
                            foldersCount: this.folders.length,
                            preselected: this.selectedHashid,
                            firstFolder: this.folders[0]
                        });

                        // Auto-open folder yang berisi alat yang sudah dipilih
                        if (this.selectedHashid && this.folders.length > 0) {
                            const folder = this.folders.find(f =>
                                f.items && f.items.some(item => item.hashid === this.selectedHashid)
                            );
                            if (folder) {
                                this.openFolders.push(folder.nama);
                                console.log('Auto-opened folder:', folder.nama);
                            }
                        }
                    },

                    // Total alat (semua varian)
                    get totalAlats() {
                        return this.folders.reduce((sum, f) => sum + (f.count || (f.items ? f.items.length : 0)), 0);
                    },

                    // Jumlah hasil filter
                    get filteredCount() {
                        return this.filteredFolders.reduce((sum, f) => sum + (f.items ? f.items.length : 0), 0);
                    },

                    // Filter folder berdasarkan search
                    get filteredFolders() {
                    ...
                    // Cek apakah folder sedang terbuka
                    isOpen(name) {
